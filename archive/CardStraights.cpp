...
class CardStraights {
	...
	int longestStraight(vector <int> c) {
		int i, jkr = 0, j, ans = 0, k, st, x;
		memset(arr, 0, sizeof arr);
		FOR(i, 0, SZ(c)) arr[c[i]] = 1;
		FOR(i, 0, SZ(c)) if(!c[i]) jkr++;
		FOR(i, 1, mx){
			if(!arr[i]) continue;
			k = jkr, st = i, x = i;
			while(x < mx){
...
namespace moj_harness {
	...
	int run_test_case(int casenum__) {
		...
		case 0: {
			...
			int received__            = CardStraights().longestStraight(vector <int>(cards, cards + (sizeof cards / sizeof cards[0])));
